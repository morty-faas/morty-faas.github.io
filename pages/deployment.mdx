...
export IFACE_IP=$(ip -f inet addr show "${IF_NET}" | awk '/inet / {print $2}' | cut -d "/" -f 1)
```
...
You can download the latest Morty binaries from their releases pages on GitHub: [controller and registry](https://github.com/morty-faas/morty/releases/latest). Here, we will install the `v1.1.0-RC1` of those components.

```bash
# Download the release artifacts
curl -sL "https://github.com/morty-faas/morty/releases/download/v1.1.0-RC1/morty-controller-v1.1.0-RC1-linux-amd64.tar.gz" --output controller.tar.gz
curl -sL "https://github.com/morty-faas/morty/releases/download/v1.1.0-RC1/morty-registry-v1.1.0-RC1-linux-amd64.tar.gz" --output registry.tar.gz
